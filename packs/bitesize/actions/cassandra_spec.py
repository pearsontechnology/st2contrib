...
class CassandraSpec(Action):
    ...
    # amiid                     - config.yaml
        ...
        try:
            ...
            template_file = self.config.get('template_path') + payload['object_kind'] + '.template'
            namespace = payload['namespace']
            environment = self.config['environment']
            template_body = open(template_file, 'r').read()
            if payload['object_kind'] == 'Cassandra' and 'version' in payload['labels']:
                if '3.0' in str(payload['labels']['version']):
