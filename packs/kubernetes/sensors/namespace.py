...
class Namespace(Sensor):
    def __init__(self, sensor_service, config=None):
        super(Namespace, self).__init__(sensor_service=sensor_service, config=config)
        self._log = self._sensor_service.get_logger(__name__)
        self.TRIGGER_REF = 'kubernetes.namespace'
        self.client = None
    ...
    def run(self):
        self._log.debug('Watch Kubernetes for new namespaces')
        r = self.client
        lines = r.iter_lines()
    ...
    def _k8s_object_to_st2_trigger(self, k8s_object):
        ...
        try:
            ...
            if name in ['default', 'kube-system']:
                self._log.info('AAAAAAA Name: %s.' % name)
                return 0
            else:
    ...
    def _build_a_trigger(self, resource_type, name, labels, object_kind, uid):
        payload = {
